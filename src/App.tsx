...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const isAuth = !!secureStorage.getItem('token');
    if (!dataFetchedRef.current && isAuth) {
      refreshCurrentUser(); // Load currentUser from session on app init
      fetchMasterData();
      fetchProfile();
  ...
  // Auth state remains local or separate auth store for now (persisted manually above)
  // Auth state
  // Auth state
  ...
  const handleLogin = () => {
    // Token and user data are set by api.login(), we just update state
    setIsAuthenticated(true);
    // Load the current user from session into the store
    refreshCurrentUser();
    // Immediately fetch organization & master data so UI updates without requiring page refresh
    try {
      fetchMasterData();
      fetchProfile();
    } catch (e) {
      // best-effort; failure will be handled by the store's internal error handling
      console.error('Failed to initialize organization data after login', e);
    }

    // Small delay to navigate to dashboard after token persistence
    setTimeout(() => {
  ...
  const [config, setConfig] = useState(() => {
    const saved = localStorage.getItem('Hunzal_config');
    return saved
      ? JSON.parse(saved)
      : {
          brandingName: 'Hunzal',
  ...
  useEffect(() => {
    ...
    window.addEventListener('Hunzal_config_updated', handleConfigUpdate);
    return () => window.removeEventListener('Hunzal_config_updated', handleConfigUpdate);
  }, []);
    ...
    <ToastProvider>
      ...
      >
        <aside
          ref={asideRef}
          className={
            `fixed h-screen left-0 top-0 transform transition-transform duration-300 bg-[#0f172a] flex flex-col z-50 shadow-2xl border-r border-blue-500/20 overflow-y-auto overscroll-contain custom-scrollbar ` +
            (sidebar.isOpen
          ...
          <div className="flex flex-col items-center justify-center mb-3 mt-5 px-6 shrink-0">
            <div className="flex items-center gap-6 mb-2">
              ...
              <div className="flex flex-col">
                <span className="font-black text-4xl tracking-tighter text-slate-100 uppercase whitespace-nowrap leading-none">
                  {config.brandingName}
                </span>
                ...
                {group.items
                  ...
                  .map((item) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3.5 py-1.5 rounded-xl transition-all duration-300 text-left group/navitem ${
                        activeModule === item.id
                          ? 'bg-blue-600 text-slate-100 shadow-md shadow-blue-600/10 border border-blue-500/20'
                    ...
                    >
                      <div
                        className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all border ${
                          activeModule === item.id
                            ? 'bg-slate-100/10 border-slate-100/20'
        ...
        >
          ...
          <header className="h-16 bg-[#0f172a] border-b border-blue-500/20 sticky top-0 z-50 px-8 flex items-center justify-between transition-all duration-300 shrink-0 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 text-sm font-medium text-slate-500">
                <span className="text-slate-200 font-black uppercase tracking-wider text-[0.8rem]">
                  {moduleLabelMap[activeModule]}
                </span>
              </div>
            </div>
